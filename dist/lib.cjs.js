...
const ArtemisContext = React.createContext();
...
function useArtemisClient() {
  return React.useContext(ArtemisContext);
}
...
function useQuery(query, opts = { variables: {} }) {
  ...
  const [variables, setVariables] = React.useState(opts.variables);

  // use to hold an `update data` function provided by `fetchMore`
  const updateQueryRef = React.useRef(null);

  // derived state from props
  // https://reactjs.org/docs/hooks-faq.html#how-do-i-implement-getderivedstatefromprops
  const [prevVars, setPrevVars] = React.useState(opts.variables);
  ...
  const op = React.useMemo(() => client.createOperation({ query, variables }), [
    query,
    variables
  ]);
  ...
  return ComposedComponent => {
    return props => {
      ...
      if (client.createOperation({ query }).type === "mutation") {
        return React__default.createElement(ComposedComponent, {
          ...props,
          mutate: () => {}
        });
      }
...
function withApollo(ComposedComponent) {
  ...
  return props => React__default.createElement(ComposedComponent, { ...props, client });
}
...
exports.ArtemisProvider = ArtemisProvider;
exports.Query = Query;
